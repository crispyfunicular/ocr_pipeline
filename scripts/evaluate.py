...
def evaluer_manuel(nom_manuel):
    ...
    for fichier in sorted(os.listdir(dossier_ref)):
        if fichier.endswith(".jsonl"):
            ...
            if ref_br and hyp_br:
                ...
                print(msg)
                detailed_lines.append(f"- **Breton** : {msg.replace('  Breton   -> ', '')}")
            ...
            if ref_fr and hyp_fr:
                ...
                print(msg)
                detailed_lines.append(f"- **Français** : {msg.replace('  Français -> ', '')}")
    ...
    if global_ref_br and global_hyp_br:
        try:
            ...
            print(msg)
            global_lines.append(f"- **Breton GLOBAL** : WER: **{g_wer_br:.1f}%**, CER: **{g_cer_br:.1f}%**")
    ...
    if global_ref_fr and global_hyp_fr:
        try:
            ...
            print(msg)
            global_lines.append(f"- **Français GLOBAL** : WER: **{g_wer_fr:.1f}%**, CER: **{g_cer_fr:.1f}%**")
        except Exception as e:
...
def main(argv=None):
    ...
    if args.targets:
        ...
        if review_dir.exists():
            manuels_a_evaluer = [
                d.name for d in review_dir.iterdir() if d.is_dir()
            ]
        else:
